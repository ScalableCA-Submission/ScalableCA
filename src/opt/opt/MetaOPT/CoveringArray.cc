...
CoveringArray::CoveringArray(const SpecificationFile &specificationFile,
                            ...
                            int seed, const int finalChk, const std::string _output_path)
...
//   }
  ...
  std::cout << "final 3-wise CA size is: " << bestArray.size() << std::endl;
  
  std::string result_path = output_path;
  // std::cout << "c Testcase set saved in " << result_path << std::endl;
  ...
  for (unsigned i = 0; i < bestArray.size(); ++i) {
    ...
    for (auto x : bestArray[i]) {
      // std::cout << x - options.firstSymbol(option) << ' ';
      res_file << x - options.firstSymbol(option) << ' ';
      option ++;
    }
    res_file << "\n";
    // std::cout << std::endl;
...
//             << array.size() << '\t' << step << std::endl;
    ...
    std::cout << "\033[;32mc current 3-wise CA size: " << array.size() 
        << " \033[0m" << std::endl;
}
